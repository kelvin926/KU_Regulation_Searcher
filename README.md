...
2. 최신 버전의 `KU-Regulation-Setup-0.2.0.exe`를 다운로드합니다.
3. 설치 파일을 실행합니다.
...
- `동기화`: 접근 가능한 규정 목록 새로고침, 검색/선택, 선택 규정 동기화
- `AI 설정`: Gemini API Key 저장/삭제, 모델 선택, 연결 테스트
- `규정 질의`: 질문 입력, 관련 조항 검색, 선택 조항 기반 답변 생성
...
4. `답변 생성`을 누르면 선택된 조항만 Gemini API로 전송합니다.
5. 답변에는 사용한 조항 ID와 근거 조항 정보가 표시됩니다.
