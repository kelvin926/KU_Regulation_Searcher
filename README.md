...
2. 최신 버전의 `KU-Regulation-Setup-0.4.0.exe`를 다운로드합니다.
3. 설치 파일을 실행합니다.
...
3. `AI 설정` 화면에서 Gemini API 키를 저장합니다.
4. `규정 동기화` 화면에서 `규정 목록 새로고침`을 누릅니다.
5. 필요한 규정을 폴더 목록에서 선택하고 `선택 규정 동기화`를 누릅니다.
6. `규정 질의` 또는 `규정 검색`에서 질문하거나 조항을 검색합니다.
...
0.3.0 이하에서 사용하던 `%APPDATA%\KU Regulation Assistant\` 데이터는 0.4.0 실행 시 새 경로로 복사됩니다.
